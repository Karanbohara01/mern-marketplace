...
// import { setSelectedUser } from "@/redux/authSlice";
// import { setMessages } from "@/redux/chatSlice"; // Import setMessages
// import { setPosts } from "@/redux/postSlice";
// import axios from "axios";
// import { Edit, MoreVertical, Send, Trash } from "lucide-react";
// import { useEffect, useState } from "react";
...
//   const { messages } = useSelector((store) => store.chat); // Get messages from Redux
...
//   const [editedLocation, setEditedLocation] = useState("");
//   const [activeTab, setActiveTab] = useState("comments"); // State to track active tab
//   const [isOfferModalOpen, setIsOfferModalOpen] = useState(false); // Offer Modal state
//   const [offerText, setOfferText] = useState(""); // Offer Text State
...
//         setIsOfferModalOpen(false);
//         setOfferText("");
//         if (selectedPost.author) dispatch(setSelectedUser(selectedPost.author));
//       }
//     } catch (error) {
//       setLoadingOffer(false);
//       console.log(error);
...
//     if (!selectedPost?.author?._id) return; // Ensure the author exists
...
//             />
//             <div className="flex justify-end gap-2">
//               <Button variant="secondary" onClick={handleCancelOfferModal}>
//                 Cancel
//               </Button>
//               <Button disabled={loadingOffer} onClick={sendOfferHandler}>
  ...
  const sendMessageHandler = async (receiverId) => {
    try {
      ...
      if (res.data.success) {
        ...
        setIsOfferModalOpen(false);
        setOfferText("");
        // Find the user from suggested users
        const offerUser = suggestedUsers?.find(
          (suggestedUser) => suggestedUser?._id === receiverId
        );
        if (offerUser) {
          dispatch(setSelectedUser(offerUser));
          navigate("/chat");
        } else {
          // Handle the case when author is not in suggested users
          console.log(
            "Author is not in suggested users, handle the situation here"
          );
        }
      }
    } catch (error) {
